...
def check_help_end(data):
    global help_indentation
    tmp = data.lstrip("\n")
    stripped_line = tmp.lstrip()
    now_indentation = len(tmp) - len(stripped_line)
    if help_indentation != 0:
        if now_indentation == help_indentation:
            return False
    return True


# 判断缩进 handle_indentation已保证help内容缩进一致
def t_HELP_CONTEXT(t):
    r'[\n|\t]+.*'
    global help_context, help_indentation
    line = t.value.lstrip("\n")
    stripped_line = line.lstrip()
    now_indentation = len(line) - len(stripped_line)
    if now_indentation != 0 and help_indentation == 0:
        help_indentation = now_indentation
    if check_help_end(t.lexer.lexdata[t.lexer.lexpos:t.lexer.lexlen]):
        t.type = "HELP_CONTEXT"
        t.value = help_context + ' ' + line
        t.lexer.begin('INITIAL')
        help_context = ""
        help_indentation = 0
        return t
...
def t_SOURCE_error(t):
    print(PATH[len(PATH) - 1] + " => source Illegal charactor '%s'" % t.value[-10:20])
    global ERROR_FLAG
    ERROR_FLAG = True
    t.lexer.skip(1)
...
if __name__ == '__main__':
    # data = '''source "drivers/video/canaan/twod/Kconfig"
    # '''
    with open("/home/guosy/Kconfig/OS/linux-6.3/Kconfig", "r") as file:
        # 读取文件内容
        data = file.read()
    lexer.input(data)
    while True:
